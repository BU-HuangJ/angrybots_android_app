	...
	public class UserLoginTask extends AsyncTask<String, Void, Boolean> {		
		...
		protected Boolean doInBackground(String... params) {			
			...
			try {
				...
				adapters.NetworkAdapter.connect("129.62.88.33");
				connected = true;
			} catch (IOException e) {
				connected = false;
			}
			...
			if (connected) {
				...
				// if logged in
				if (adapters.NetworkAdapter.getMember() != null) {
					...
					adapters.PersistentSettings.prefs.achievementTopHat    = member.getAchievementMap().containsKey(0);
					adapters.PersistentSettings.prefs.achievementCowboyHat = member.getAchievementMap().containsKey(1);
					adapters.PersistentSettings.prefs.achievementMonocle   = member.getAchievementMap().containsKey(2);
					adapters.PersistentSettings.prefs.achievementMustache  = member.getAchievementMap().containsKey(3);
					
					String faction = adapters.NetworkAdapter.getMember().getaFaction().getName();
